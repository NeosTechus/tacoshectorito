...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!ownerEmail) {
      logWarn('owner_email_missing', requestContext);
      return res.status(200).json({ success: true, skipped: 'No OWNER_EMAIL configured' });
    }
    ...
    if (error) {
      logError('owner_email_error', error, requestContext);
      return res.status(500).json({ error: error.message });
    }

    logInfo('owner_email_sent', { ...requestContext, ownerEmail, messageId: data?.id });
